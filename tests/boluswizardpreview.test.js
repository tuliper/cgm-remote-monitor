...
describe('boluswizardpreview', function ( ) {
 ...
 it('should calculate IOB results correctly with 1.0 U IOB resulting in going low in MMOL', function (done) {
    ...
    ddata.sgvs = [{mills: before, mgdl: 100}, {mills: now, mgdl: 100}];
    ddata.treatments = [{mills: now, insulin: '1.0'}];
    ddata.profile = require('../lib/profilefunctions')([profileData]);
    var sbx = sandbox.clientInit(ctx, Date.now(), ddata);
    var iob = require('../lib/plugins/iob')();
    sbx.properties.iob = iob.calcTotal(ddata.treatments, ddata.profile, now);
 ...
 it('should calculate IOB results correctly with 0.45 U IOB resulting in going low in MMOL', function (done) {
    ...
    var ddata = require('../lib/data/ddata')();
    ddata.sgvs = [{mills: before, mgdl: 175}, {mills: now, mgdl: 153}];
    ddata.treatments = [{mills: now, insulin: '0.45'}];
    ddata.profile = require('../lib/profilefunctions')([profileData]);
    var sbx = sandbox.clientInit(ctx, Date.now(), ddata);
    var iob = require('../lib/plugins/iob')();
    sbx.properties.iob = iob.calcTotal(ddata.treatments, ddata.profile, now);
  ...
  it('set a pill to the BWP with infos', function (done) {
    ...
    var ddata = require('../lib/data/ddata')();
    ddata.sgvs = [{mills: before, mgdl: 295}, {mills: now, mgdl: 300}];
    ddata.treatments = [{mills: before, insulin: '1.5'}];
    ddata.profile = oadedProfile;

    var sbx = require('../lib/sandbox')().clientInit(ctx, Date.now(), ddata);
